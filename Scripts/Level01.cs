...
public class Level01 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Mission.Mission.LEVELPIVOT = 0; // initilize objective
        Mission.Mission.IS_OBJ_DONE = false;
        Mission.Mission.ISFAIL = false;
        Time.timeScale = 1;
        character.HEALTH = 100;

        listener = InputKeyListener.GetListener();
	...
	void Update()
    {
        InputPause(); // PAUSE SCREEN: If the user press the "ESC" command on their keyboard, the game pauses.

        if (Mission.Mission.ISFAIL == true)
        {
        ...
        if ((Mission.Mission.ISFAIL == false) && (isDone == false))
        {
            ...
            VisorControl(); // Controls the visor

            // This statement will change the level's objective
            switch (Mission.Mission.LEVELPIVOT)
            {
                case 0:
                    isObjective = true;
                    if (Input.GetKeyDown(KeyCode.V))
                    {
                        ...
                        Mission.Mission.IS_OBJ_DONE = true;
                        Mission.Mission.LEVELPIVOT = 1; // Next objective (1)
            ...
            if (Mission.Mission.LEVELPIVOT >= 2)
            {
                ...
                if (isCamera == true)
                {
                    if (Input.GetMouseButtonDown(0))
                    {
                        ...
                        {
                            Mission.Mission.IS_OBJ_DONE = true;
                            Mission.Mission.LEVELPIVOT = 3; // Next objective (3)
                            GetComponent<AudioSource>().PlayOneShot(sound_beep_high);
                            GetComponent<AudioSource>().Play();
                        }
                    }
    ...
    void OnGUI()
    {
        ...
        if ((Mission.Mission.LEVELPIVOT >= 2) && (Mission.Mission.ISFAIL == false))
        {
            VisorWarning();
        }
        ...
        {
            // Display the health meter, objectives, and other UI elements
            LevelPlayGui();
        }
    }
    ...
    // Displays the warning message if the user turns off the visor while playing the mission.
    // Cyclomatic Complexity: 2
    private void VisorWarning()
    {
    ...
    private void InputPause()
    {
        if (Input.GetKeyDown(KeyCode.Escape) || Input.GetKeyDown(KeyCode.Joystick1Button16))
        {
            ...
            Mission.Mission.ISMENU = !Mission.Mission.ISMENU; //toggles the pause menu
        }
    }

    // Cyclomatic Complexity: 2
    private void VisorControl()
    {
        if (Input.GetKeyDown(KeyCode.V))
        {
            isVisor = !isVisor;
    ...
    private void Section01()
    {
        ...
        if ((isVisor == true) && (Mission.Mission.LEVELPIVOT == 1))
        {
            ...
            if (Input.GetMouseButton(0))
            {
                ...
                if (hit)
                {
                    if (hitInfo.collider.gameObject.CompareTag("guide01_hatch"))
                    {
                        ...
                        Mission.Mission.IS_OBJ_DONE = true;
                        Mission.Mission.LEVELPIVOT = 2; // Next objective (2)
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (hit)
            {
                if (hitInfo.collider.gameObject.CompareTag("guide03_camera"))
                {
                    ...
                    Mission.Mission.IS_OBJ_DONE = true;
                    Mission.Mission.LEVELPIVOT = 4; // Next objective (4)
